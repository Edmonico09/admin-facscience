...
export async function createNews(data : Actuality) {
...
export async function updateNews(id: number, data : Actuality) {
    try {

        const response = await fetch(`${baseURL}/actuality/update/${id}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(data),
        });
...
export async function getNews():  Promise<Actuality[]>{
    try {
...
export async function updateStatus(idActualite : number , statut : string) : Promise<Actuality> {
  try {
    const data = {
        "statut" : statut
    }
